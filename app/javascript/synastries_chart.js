// Initialize synastry chart when Turbo loads the page
document.addEventListener('turbo:load', () => {
  initSynastryChart();
});

function initSynastryChart() {
  const canvas = document.getElementById('synastryChartCanvas');
  if (!canvas) return;
  ...
  const drawChart = (timestamp) => {
    ...
    // Define wheel radii
    const innerRadius = radius * 0.8; // Inner wheel (first birth chart)
    const middleRadius = radius; // Middle wheel (first birth chart outer edge)
    const outerRadius = radius * 1.2; // Outer wheel (second birth chart)
    
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    
    // Get chart data from data attributes (moved inside drawChart)
    const birthChart1Planets = JSON.parse(canvas.dataset.birthChart1Planets || '[]');
    const birthChart1Houses = JSON.parse(canvas.dataset.birthChart1Houses || '[]');
    const birthChart1ChartPoints = JSON.parse(canvas.dataset.birthChart1ChartPoints || '[]');
    
    const birthChart2Planets = JSON.parse(canvas.dataset.birthChart2Planets || '[]');
    const birthChart2Houses = JSON.parse(canvas.dataset.birthChart2Houses || '[]');
    const birthChart2ChartPoints = JSON.parse(canvas.dataset.birthChart2ChartPoints || '[]');
    ...
    // Find the longitude of House 1 (Ascendant) for first birth chart
    const house1Chart1 = birthChart1Houses.find(h => h.house === 1);
    const house1Longitude = house1Chart1 ? house1Chart1.longitude : 0;
    ...
    // Draw zodiac wheel sectors for outer wheel (second birth chart)
    zodiacData.forEach((zodiac, i) => {
    ...
    // Draw the main middle chart circle (first birth chart outer wheel)
    ctx.beginPath();
    ...
    // Draw the outer chart circle (second birth chart wheel)
    ctx.beginPath();
    ...
    // Draw house delimiters outside the middle wheel (first birth chart)
    birthChart1Houses.forEach((house, i) => {
    ...
    // Draw house delimiters outside the outer wheel (second birth chart)
    birthChart2Houses.forEach((house, i) => {
      const adjustedLongitude = (house.longitude - house1Longitude + 360) % 360;
      const angle = ((180 - adjustedLongitude) * Math.PI) / 180;
    ...
    // Draw first birth chart houses (relative to House 1 at 0°, rotated)
    birthChart1Houses.forEach((house, i) => {
      const adjustedLongitude = (house.longitude - house1Longitude + 360) % 360;
      const angle = ((180 - adjustedLongitude) * Math.PI) / 180;
    ...
    // Draw second birth chart houses
    birthChart2Houses.forEach((house, i) => {
      const adjustedLongitude = (house.longitude - house1Longitude + 360) % 360;
      const angle = ((180 - adjustedLongitude) * Math.PI) / 180;
    ...
    // First pass: identify overlapping planets and assign padding levels for first birth chart
    const planetPadding = new Map();
    
    birthChart1Planets.forEach((planet1, i) => {
      ...
      birthChart1Planets.forEach((planet2, j) => {
        if (i !== j) {
    ...
    // Second pass: draw first birth chart planets with assigned padding
    birthChart1Planets.forEach((planet, i) => {
    ...
    // Handle overlapping planets for second birth chart
    const chart2PlanetPadding = new Map();
    
    birthChart2Planets.forEach((planet1, i) => {
      if (chart2PlanetPadding.has(planet1.planet)) return;
      
      const overlappingGroup = [planet1];
      birthChart2Planets.forEach((planet2, j) => {
        if (i !== j) {
          const diff = Math.abs(planet1.longitude - planet2.longitude) % 360;
          const minDiff = Math.min(diff, 360 - diff);
          if (minDiff < 5) {
      ...
      overlappingGroup.forEach((planet, index) => {
        if (!chart2PlanetPadding.has(planet.planet)) {
          if (overlappingGroup.length > 1) {
            ...
            chart2PlanetPadding.set(planet.planet, padding);
          }
        }
      });
    });

    // Draw second birth chart planets on outer wheel
    birthChart2Planets.forEach((planet, i) => {
      // Apply same adjustment as zodiac signs for alignment
      const adjustedLongitude = (planet.longitude - house1Longitude + 360) % 360;
      const angle = ((180 - adjustedLongitude) * Math.PI) / 180;
      
      const assignedPadding = chart2PlanetPadding.get(planet.planet) || 80; // Increased default padding
      ...
      // Draw planet symbol (more transparent for second chart)
    ...
    // Draw first birth chart points (relative to House 1 at 0°, rotated)
    const pointSymbols = {
      'Ascendant': 'AS',
      'MC': 'MC',
      'Vertex': 'Vx',
    ...
    birthChart1ChartPoints.forEach((point, i) => {
    ...
    // Draw second birth chart points 
    birthChart2ChartPoints.forEach((point, i) => {
      // Apply same adjustment as zodiac signs for alignment
      const adjustedLongitude = (point.longitude - house1Longitude + 360) % 360;
      const angle = ((180 - adjustedLongitude) * Math.PI) / 180;
    ...
    // Draw synastry aspects between the two birth charts
    birthChart1Planets.forEach((planet1, i) => {
      birthChart2Planets.forEach((planet2, j) => {
        ...
        aspects.forEach(aspect => {
          if (Math.abs(aspectAngle - aspect.angle) <= aspect.tolerance) {
            ...
            // Visual styling by aspect type for synastry
            switch (aspect.type) {
              case 'conjunction':
